...
<script>
import { filedGetInformation, filedUpdateTableField, filedGetField, filedValidates } from '@/api/customermanagement/common'
...
export default {
  ...
  methods: {
     ...
     */
    editClick(item) {
      let value = isArray(item.value) || isObject(item.value) ? objDeepCopy(item.value) : item.value
