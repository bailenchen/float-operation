  ...
  methods: {
    ...
    getList() {
      const loading = Loading.service({
        target: document.querySelector(`.el-dialog[aria-label="转移至其他公海"]`)
      })
      crmCustomerPoolNameListAPI()
        .then(res => {
          this.list = res.data || []
    ...
    handleConfirm() {
      if (this.selectId) {
        ...
        TransferCustomerAPI({
          customerId: this.selectionList.map(function(item, index, array) {
            return item.customerId
          }).join(','),
          inPoolId: this.selectId
        })
          .then(res => {
            this.$message({
              type: 'success',
            ...
            loading.close()
            this.$emit('handle', { type: 'change_seas' })
            this.handleCancel()
          })
          .catch(() => {
            loading.close()
