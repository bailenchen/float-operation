<template>
  <div v-loading="loading">
    <div class="content-title">
      <span>LEADS状态&跟进结果设置</span>
    </div>
    <div class="product-setting">
      <div class="tips">注：1、LEADS状态&跟进结果设置最多设置2级</div>
      <div class="tips">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;2、跟进结果中“承诺到访”“已到访”请勿更改。</div>
...
<script>
import {
  QueryLeadsStatusFollowResult,
  DelLeadsStatusFollowResult,
  AddLeadsStatusFollowResult
} from '@/api/systemManagement/params'

export default {
  name: 'LeadsStatusFollowResult',
  ...
  methods: {
     ...
     */
    getChild(node) {
      console.log(node, 'jjjjj')
      const temps = [
        { type: 'create-child', name: '新建跟进结果' },
        { type: 'create-brother', name: '新建LEADS状态' },
        { type: 'edit', name: '编辑LEADS状态' },
        { type: 'delete', name: '删除LEADS状态' }
      ]
      if (node.level < this.maxCreateLevel) {
        ...
        this.treeSetTypes = temp.map(item => {
          item['node'] = node
          return item
        })
      }
    ...
    handleProduct() {
      ...
      this.loading = true
      AddLeadsStatusFollowResult(params)
        .then(res => {
          this.getProductCategoryIndex()
