<template>
  <div class="register-new-company">
    <get-sms-code v-show="!showNext" ref="smsCode" />

    <el-form v-show="showNext">
      <el-form-item>
        <el-input
          ref="company_name"
          v-model="form.company_name"
          :class="{error: !validateRes.company_name}"
          placeholder="请输入企业名称(必填)"
          type="text"
          @focus="focusKey = 'company_name'"
          @blur="checkFromItem('company_name', form.company_name)">
          <span
            slot="prefix"
            :class="{focus: focusKey === 'company_name'}"
            class="form-icon wukong wukong-qiye"
            style="font-size: 14px" />
        </el-input>
      </el-form-item>

      <el-form-item>
        <el-input
          ref="company_credit_number"
          v-model="form.company_credit_number"
          :class="{error: !validateRes.company_credit_number}"
          placeholder="请输入企业信用号(必填)"
          type="text"
          @focus="focusKey = 'company_credit_number'"
          @blur="checkFromItem('company_credit_number', form.company_credit_number)">
          <span
            slot="prefix"
            :class="{focus: focusKey === 'company_credit_number'}"
            class="form-icon wukong wukong-xinyong" />
        </el-input>
      </el-form-item>

      <el-form-item>
        <el-popover
          v-model="pwdPopover"
          :width="width"
          placement="bottom-start"
          trigger="focus">
          <div class="pwd-popover-content">
            <div class="box">
              <span class="text">密码强度</span>
              <i
                v-for="i in 4"
                :key="i"
                :class="{active: i <= rankIndex}"
                class="item" />
            </div>
            <div class="desc">
              <span class="text">设置为8~20位字母、数字、特殊字符组合的密码</span>
              <span v-if="rankIndex >= 3" class="icon wukong wukong-tongguo" />
              <span v-else class="icon error wukong wukong-guanbi" />
            </div>
          </div>
          <div slot="reference" class="pwd-popover-control">
            <el-input
              ref="password"
              v-model="form.password"
              :maxlength="20"
              :class="{error: !validateRes.password}"
              placeholder="请输入密码"
              type="password"
              @input.native="calcRank"
              @focus="focusKey = 'password'"
              @blur="checkFromItem('password', form.password)">
              <span
                slot="prefix"
                :class="{focus: focusKey === 'password'}"
                class="form-icon wukong wukong-mima" />
            </el-input>
          </div>
        </el-popover>
      </el-form-item>

      <el-form-item>
        <el-input
          ref="re_password"
          v-model="form.re_password"
          :maxlength="20"
          :class="{error: !validateRes.re_password}"
          placeholder="请输入确认密码"
          type="password"
          @focus="focusKey = 're_password'"
          @blur="checkForm">
          <span
            slot="prefix"
            :class="{focus: focusKey === 're_password'}"
            class="form-icon wukong wukong-mima" />
        </el-input>
      </el-form-item>
    </el-form>

    <div :class="{ok: !Boolean(errorInfo)}" class="error-info">
      <div v-if="errorInfo" class="box">
        <img src="~@/assets/login/error.png" alt="" class="icon">
        <span>{{ errorInfo }}</span>
      </div>
    </div>

    <div class="control">
      <div v-if="!showNext" class="btn" @click="handleNext">
        下 一 步
      </div>
      <div v-else class="btn" @click="handleRegister">
        注&nbsp;&nbsp;&nbsp;册
      </div>
    </div>

    <div class="to-login">
      已有账号，<span class="special" @click="$emit('toggle', 'LoginByPwd')">去登录</span>
    </div>
  </div>
</template>

<script>
  import {RegisterAPI} from '@/api/login'

  import GetSmsCode from './getSmsCode'
  import mixins from './mixins'

  export default {
    name: 'LoginByCode',
    components: {
      GetSmsCode
    },
    mixins: [mixins],
    data() {
      const pwdReg = /^(?=.*[a-zA-Z])(?=.*\d)(?=.*[~!@#$%^&*()_+`\-={}:";'<>?,.\/]).{8,20}$/;
      return {
        form: {
          password: null
        },
        validateRes: {
          company_name: true,
          company_credit_number: true,
          password: true,
          re_password: true,
        },
        errorInfo: '',

        showNext: false,

        rules: {
          company_name: [
            {required: true, msg: '企业名称不能为空'}
          ],
          company_credit_number: [
            {required: true, msg: '企业信用号不能为空'}
          ],
          password: [
            {required: true, msg: '密码不能为空'},
            {reg: pwdReg, msg: '密码必须由8-20位字母、数字、特殊符号组成'}
          ]
        },

        pwdPopover: false, // 密码强度 popover
        width: 0, // 密码强度 popover 宽度
        rankIndex: 0 // 密码强度等级
      }
    },
    mounted() {
      this.$nextTick(() => {
        this.getWidth()
      })
    },
    methods: {
      getWidth() {
        this.width = document.getElementsByClassName('el-form-item')[0].clientWidth
      },
      /**
       * 下一步
       */
      handleNext() {
        this.clearError()
        let data = this.$refs.smsCode.getData()
        if (!data) return
        this.form = {
          ...this.form,
          ...data.form
        }
        this.showNext = true
      },

      /**
       * 注册
       */
      handleRegister() {
        let flag = this.checkForm()
        if (!flag) return
        let params = Object.assign({}, this.form)
        if (params.hasOwnProperty('re_password')) {
          delete params.re_password
        }

        console.log('register', params)
        RegisterAPI(params).then(() => {
          this.$message.success('注册成功')
          this.$emit('toggle', 'LoginByPwd')
        }).catch()
      },

      /**
       * 注册前表单校验
       * @return {boolean}
       */
      checkForm() {
        this.clearError()
        let arr = ['company_name', 'company_credit_number', 'password']
        for (let i = 0; i < arr.length; i++) {
          let res = this.checkFromItem(arr[i], this.form[arr[i]] || null)
          if (!res) return false
        }
        if (this.form.password !== this.form.re_password) {
          this.setError('re_password', '两次密码输入不一致')
          return false
        }
        console.log('ok')
        return true
      },

      clearError() {
        this.validateRes = {
          company_name: true,
          company_credit_number: true,
          password: true,
          re_password: true,
        }
        this.errorInfo = null
      },

      /**
       * 密码强度计算
       */
      calcRank() {
        let pwd = this.form.password
        if (pwd.length < 8) {
          this.rankIndex = 0
          return
        }
        // let reg1 = /[A-Z]/; // 至少一个大写字母
        // let reg2 = /[a-z]/; // 至少一个小写字母
        // let reg3 = /[0-9]/; // 至少一个数字
        // let reg4 = /[~!@#$%^&*()_+`\-={}:";'<>?,.\/]/; // 至少一个特殊字符

        let regArr = [/[A-Z]/, /[a-z]/, /[0-9]/, /[~!@#$%^&*()_+`\-={}:";'<>?,.\/]/]
        let resArr = []
        regArr.forEach(reg => {
          resArr.push(reg.test(pwd))
        })
        this.rankIndex = resArr.filter(o => o).length
      },
    }
  }
</script>

<style scoped lang="scss">
  @import "../index";
</style>
